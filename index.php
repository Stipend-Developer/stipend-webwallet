	...
	if (!empty($_POST['jsaction'])) {
		...
		$json['balance'] = $jsonbal;
                ...
                if (!WITHDRAWALS_ENABLED) {
                    ...
                    $json['message'] = "Withdrawal amount exceeds your wallet balance with a 0.0001 SPD transaction fee added";
                } else {
        ...
        if (!empty($_POST['action'])) {
            switch ($_POST['action']) {
                ...
                if (!WITHDRAWALS_ENABLED) {
                    ...
                    $error['type'] = "withdraw";
                    $error['message'] = "Withdrawal amount exceeds your wallet balance with a 0.0001 SPD transaction fee added";
                } else {
                    $withdraw_message = $client->withdraw($user_session, $_POST['address'], (float)$_POST['amount']);
                    $_SESSION['token'] = sha1('@s%a$l£t#'.rand(0,10000));
                    header("Location: index.php");
